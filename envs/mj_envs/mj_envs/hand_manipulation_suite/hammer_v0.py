...
from envs.mj_envs.mj_envs.utils.quatmath import *
import os
...
class HammerEnvV0(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def reset_model(self):
        self.sim.reset()
        self.sim.data.qpos[:-9] += np.random.normal(0, 0.05, self.sim.data.qpos[:-9].shape[0])
        target_bid = self.model.body_name2id('nail_board')
        self.model.body_pos[target_bid,2] = self.np_random.uniform(low=0.1, high=0.25)
        self.sim.forward()
        # np.random.normal(10, 10, self.init_qpos.shape[0])
    ...
    def evaluate_success(self, paths):
        ...
        return success_percentage, rewards
