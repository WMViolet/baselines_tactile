...
MANIPULATE_EGG_XML = os.path.join('hand', 'soft_manipulate_egg.xml')
# MANIPULATE_PEN_XML = os.path.join('hand', 'manipulate_pen.xml')


class SoftManipulateEnv(hand_env.HandEnv):
    def __init__(
        ...
        ignore_z_target_rotation=False, obs_type = 'original', fixed_num_of_contact = 5,
    ):
        ...
        self.hand_geoms = list(range(1, 46))
        ...
        self.contact_dim = (self.sim.model.ngeom + 9) * fixed_num_of_contact
    ...
    def _get_obs(self):
        ...
        if self.obs_type == 'object':
            ...
            if self.obs_type == 'contact':
                observation = np.concatenate((processed, object_qvel, achieved_goal), -1)
            elif self.obs_type == 'full_contact':
                observation = np.concatenate((processed, observation), -1)
            elif self.obs_type == 'joint_contact':
                observation = np.concatenate((processed, robot_qpos, robot_qvel), -1)
        return {
...
class SoftHandEggEnv(SoftManipulateEnv, utils.EzPickle):
    def __init__(self, target_position='random', target_rotation='xyz', reward_type='sparse', obs_type = 'original', fixed_num_of_contact = 0):
        utils.EzPickle.__init__(self, target_position, target_rotation, reward_type)
        # self.object_indices = [46] + list(range(48, 82))
        self.object_indices = [46] + list(range(48, 240))
        SoftManipulateEnv.__init__(self,
