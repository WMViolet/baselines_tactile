...
from tactile_baselines.her.ddpg import DDPG
from tactile_baselines.her.her_sampler import make_sample_her_transitions
from tactile_baselines.bench.monitor import Monitor
...
DEFAULT_PARAMS = {
    ...
    'network_class': 'tactile_baselines.her.actor_critic:ActorCritic',
...
def cached_make_env(make_env, obs_type = 'original', fixed_num_of_contact = 0):
    """
    ...
    if make_env not in CACHED_ENVS:
        env = make_env(obs_type = obs_type, fixed_num_of_contact = fixed_num_of_contact)
...
def prepare_params(kwargs):
    ...
    tmp_env = cached_make_env(kwargs['make_env'], kwargs['obs_type'], kwargs['fixed_num_of_contact'])
...
def configure_her(params):
    env = cached_make_env(params['make_env'], params['obs_type'], params['fixed_num_of_contact'])
    env.reset()
...
def configure_ddpg(dims, params, process_type='none', reuse=False, feature_dim=64,
                   use_mpi=True, clip_return=True, fixed_num_of_contact = 0, include_action = False,
                   contact_dim=0, pre_train_model=False, feature_net_path='', update_model=True):
