...
class SAC(object):
    ...
    def __init__(self, input_dims, buffer_size, hidden, layers, network_class, polyak, batch_size,
        ...
        with tf.variable_scope(self.scope):
            ...
            if self.pre_train_model == 'feature_net':
    ...
    def store_episode(self, episode_batch, update_stats=True):
        ...
        # noise = np.random.normal(2e-4, 1e-5, ori.shape)
    ...
    def stage_batch(self, batch=None, train_supervised_model=False):
        if batch is None:
    ...
    def train(self, stage=True, train_supervised_model=False):
        if stage:
            if self.pre_train_model == 'feature_net':
                feature_loss = self.stage_batch(train_supervised_model=train_supervised_model)
            else:
                self.stage_batch()
    ...
    def _create_network(self, reuse=False):
        ...
        self.net = self.create_actor_critic(batch_tf, **self.__dict__)
        # networks
        # batch_tf ['g', 'o', 'u', 'o_2', 'g_2', 'r'])
