...
class TfNormalizer:
    def __init__(self, size, eps=1e-2, default_clip_range=np.inf, sess=None):
        ...
        """ Normalize the predicted object positions, same as the original Normalizer except
        that the inputs are always tensors.
    ...
    # def update(self, v):
    #     v = tf.reshape(v, [-1, self.size])
    #
    #     with self.lock:
    #         self.local_sum += tf.reduce_sum(v, axis = 0)
    #         self.local_sumsq += tf.reduce_sum(tf.square(v), axis = 0)
    #         self.local_count += tf.cast(tf.shape(v)[0], tf.float32)
    ...
    def recompute_stats(self):
        with self.lock:
            ...
            # local_count = tf.identity(self.local_count)
            # local_sum = tf.identity(self.local_sum)
            # local_sumsq = tf.identity(self.local_sumsq)

            # self.sess.run(self.reset_op)

            local_count, local_sum, local_sumsq = self.sess.run([self.local_count,
                                                                 self.local_sum,
                                                                 self.local_sumsq])
            self.local_sum = tf.zeros(self.size, tf.float32)
            self.local_sumsq = tf.zeros(self.size, tf.float32)
