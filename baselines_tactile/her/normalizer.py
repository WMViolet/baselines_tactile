...
from tactile_baselines.her.util import reshape_for_broadcasting
...
class Normalizer:
    ...
    def save(self, save_path):
        var_list = self.sess.run([self.sum_tf, self.sumsq_tf, self.count_tf, self.mean, self.std])
        st()
        with open(save_path, 'wb') as pickle_file:
            pickle.dump(var_list, pickle_file)

    def restore(self, load_path):
        sum_tf, sumsq_tf, count_tf, mean, std = pickle.load(open(load_path, 'rb'))
        restored_value = tf.constant(sum_tf), tf.constant(sumsq_tf), tf.constant(count_tf), tf.constant(mean), tf.constant(std)
        var_list = [self.sum_tf, self.sumsq_tf, self.count_tf, self.mean, self.std]
        st()
        self.sess.run([tf.assign(var, restored) for (var, restored) in zip(var_list, restored_value)])
